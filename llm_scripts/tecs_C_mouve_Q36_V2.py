...
class SimulationConfig:
    ...
    real_time: bool = True
    # Additional slots for elements (extracted for convenience)
    slot_presets: dict = field(default_factory=lambda: {
        "A": [0, 4, 5],
        "B": [2, 3],
        "D": [1]
    })
...
if __name__ == "__main__":
    ...
    elements = [
        Element("A1", k=2, start_pos=0, allowed_slots=config.slot_presets["A"], 
                set_name="A", priority=10, stall_probability=0.1, aging_factor=0.8),
        Element("A2", k=1, start_pos=4, allowed_slots=config.slot_presets["A"], 
                set_name="A", priority=5, stall_probability=0.2, aging_factor=1.2),
        Element("A3", k=3, start_pos=5, allowed_slots=config.slot_presets["A"], 
                set_name="A", priority=8, stall_probability=0.05, aging_factor=0.5),
        Element("B1", k=1, start_pos=2, allowed_slots=config.slot_presets["B"], 
                set_name="B", priority=7, stall_probability=0.15, aging_factor=1.0),
        Element("B2", k=2, start_pos=3, allowed_slots=config.slot_presets["B"], 
                set_name="B", priority=3, stall_probability=0.0, aging_factor=0.3),
        Element("D1", k=1, start_pos=1, allowed_slots=config.slot_presets["D"], 
                set_name="D", priority=15, stall_probability=0.0, aging_factor=0.0, weight=2)
    ]
